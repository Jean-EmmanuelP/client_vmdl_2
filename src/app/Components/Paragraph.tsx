...
export default function Paragraph({ section, children, bgColor, textColor, className, classTitle, classText, homeSection }: ParagraphProps) {
  const { currentSection, setCurrentSection, setHeaderHeight } = useSection();
  const [animateEntry, setAnimateEntry] = useState<number[]>([0, 0, 0, 0, 0, 0]);
  const [toggle, setToggle] = useState<boolean>(false);
  const [previousState, setPreviousState] = useState<boolean>(false);
  const [isHovering, setIsHovering] = useState<boolean>(false);

  const mergedClass = twMerge('relative w-full h-full flex flex-col justify-center items-center gap-4 pt-4', className);
  const mergedTitle = twMerge(`text-[40px] ${toggle && 'absolute left-2'}`, classTitle);
  ...
  return (
    ...
    <motion.div
      ...
      animate={{ opacity: 1, backgroundColor: toggle ? oppositeBg : bgColor, color: toggle ? oppositeText : textColor }}
      transition={{
        opacity: { duration: 0.5 },
        backgroundColor: { duration: !toggle && previousState ? 1.5 : .8 },
      ...
      <Fragment key={`${section}-paragraph`}>
        ...
        <motion.button
          ...
          animate={{ y: animateEntry[section] ? (toggle ? '-40px' : 0) : '40px', x: toggle ? '-0px' : 0, opacity: animateEntry[section] ? 1 : 0 }}
          transition={{ delay: !toggle && !previousState ? 0.5 : 0, duration: 0.5 }}
        ...
        <motion.p
          ...
          animate={{ y: animateEntry[section] ? (toggle ? '-10px' : 0) : '40px', opacity: animateEntry[section] ? (!toggle ? 1 : 0) : 0 }}
          // transition={{ delay: !toggle && !previousState ? 0.6 : 0, duration: 0.2 }}
          transition={{
            opacity: { delay: !toggle && !previousState ? 0.6 : 0, duration: 0.5 },
            delay: !toggle && !previousState ? 0.6 : 0, duration: 0.5
          }}
        ...
        {children.length > 2 &&
          <motion.button
            ...
            animate={{ y: animateEntry[section] ? 0 : '40px', opacity: animateEntry[section] && !toggle ? 1 : 0, backgroundColor: isHovering ? '' : '' }}
            transition={{
              backgroundColor: { delay: 0, duration: 0.3 },
              delay: toggle && !previousState ? 0 : 0.7, duration: 0.5
            }}
            onClick={() => {
              if (section > 0)
                setToggle(true)
            ...
            className={`text-[#181a1b] p-4 ${!homeSection ? `rounded-full w-[280px] hover:bg-gray-500/5 transition duration-150` : `text-white tracking-wide rounded-md bg-gray-600 bg-clip-padding backdrop-filter backdrop-blur-md bg-opacity-20 border border-gray-100/20 shadow-2xl p-2 sm:p-4 w-[169px] sm:w-[220px]`} text-sm uppercase flex justify-center items-center `}>
            <motion.span animate={{ x: isHovering ? '0' : '8px' }}
              transition={{ duration: 0.5 }}
            >{children[2]}</motion.span>
            <motion.span
              initial={{ opacity: 0, paddingLeft: '0px' }}
              animate={{ opacity: isHovering ? 1 : 0, paddingLeft: isHovering ? '10px' : '0px' }}
