// Floating quick-contact buttons (WhatsApp + Phone). Pinned to the
// bottom-left of every page, always tappable. Plain server component (no
// JS needed) so it renders on first paint and is crawlable.
...
export default function WhatsAppFab() {
  ...
  return (
    <div className="fixed left-4 bottom-4 sm:left-6 sm:bottom-6 z-[2147483646] flex flex-col gap-3">
      <a
        href={waHref}
        target="_blank"
        rel="noopener noreferrer"
        aria-label="Contacter Maître Vincent Machado Da Luz sur WhatsApp"
        data-clickable="true"
        className="flex items-center gap-2 rounded-full bg-[#25D366] text-white shadow-2xl shadow-black/30 hover:scale-105 hover:bg-[#1ebe5d] active:scale-95 transition-all duration-300 ease-out h-12 w-12 sm:h-14 sm:w-14 justify-center"
      >
      ...
      <a
        href={telHref}
        aria-label={`Appeler le cabinet VMDL au ${PHONE_DISPLAY}`}
        data-clickable="true"
        className="flex items-center gap-2 rounded-full bg-noir text-blanc shadow-2xl shadow-black/30 hover:scale-105 hover:bg-[#1a1a1a] active:scale-95 transition-all duration-300 ease-out h-12 w-12 sm:h-14 sm:w-14 justify-center"
      >
        {/* Inline phone glyph */}
        <svg
          viewBox="0 0 24 24"
