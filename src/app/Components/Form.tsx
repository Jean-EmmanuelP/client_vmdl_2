...
export default function FormContact() {
    ...
    const langCodeMap: { [key in LangueCode]: string } = {
      ...
      const { nom_prenom, Numero_de_telephone, Courriel, Message, Envoyer } = data[langCode].contact;
    ...
    const validationMessages = {
      ...
      const messages = validationMessages[langCode as keyof typeof validationMessages] || validationMessages['fr'];
      const validationSchema = Yup.object({
        nom: Yup.string().required(messages.required),
        email: Yup.string().email(messages.email).required(messages.required),
        telephone: Yup.string().matches(/^[0-9]+$/, messages.phone).required(messages.required),
    ...
    return (
        ...
        <div className="fixed bg-white py-[10%] px-[5%] w-1/2 h-1/2 border shadow-2xl rounded-md top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex gap-1 items-center text-center text-2xl z-10">
            ...
            <h1 className="font-bold text-2xl">Nous accusons bonne reception de votre message</h1>
        ...
        <div className="flex flex-col gap-2">
            ...
            <div className="flex justify-center items-center">
              <motion.button
                ...
                animate={{ y: '40px', opacity: 1, backgroundColor: isHovering ? '#F0F0F0' : '#FFFFFF' }}
                transition={{
                  backgroundColor: { delay: 0, duration: 0.3 },
                  delay:  0.7, duration: 0.5
                }}
                type="submit"
                className='text-[#181a1b] p-4 rounded-full w-[280px] text-sm uppercase flex justify-center items-center'
              >
