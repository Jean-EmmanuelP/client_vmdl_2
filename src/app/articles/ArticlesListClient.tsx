      ...
      <li key={article.slug} className="group">
        ...
        >
          <div className="sm:w-40 flex-shrink-0 mb-3 sm:mb-0">
            <p className="text-[11px] tracking-[0.25em] text-noir/50">
              {formatDateLocale(article.publishedAt, localeDateMap[lang])}
            </p>
            <p className="text-[11px] tracking-[0.2em] text-noir/35 mt-1">
              {readingTime(data.content)} {t.minRead}
            </p>
          </div>
          <div className="flex-1">
            <h2 className="text-[18px] sm:text-[24px] font-light leading-[1.2] group-hover:text-noir transition-colors">
              {data.title}
            </h2>
            {data.excerpt && (
            ...
            <span className="inline-block mt-4 text-[10px] tracking-[0.3em] text-noir/40 group-hover:text-noir transition-colors">
              {t.readArticle} →
            </span>
          </div>
        </Link>
  ...
  return (
    <section className="max-w-5xl mx-auto px-6 sm:px-10 pt-28 sm:pt-36 pb-20">
      <header className="mb-12 sm:mb-16 border-b border-noir/15 pb-8">
        <p className="text-[11px] sm:text-xs tracking-[0.3em] text-noir/50 mb-3">
          {t.cabinetTitle}
        </p>
        <h1 className="text-[28px] sm:text-[42px] leading-[1.1] font-light">
        ...
        {(penal.length > 0 || football.length > 0) && (
          <nav
            aria-label="Sections"
            className="mt-6 flex flex-wrap gap-x-6 gap-y-2 text-[10px] sm:text-[11px] tracking-[0.25em] text-noir/55"
          >
            {penal.length > 0 && (
      ...
      {articles.length === 0 ? (
        <div className="py-24 text-center">
          <p className="text-xs tracking-[0.3em] text-noir/40 mb-4">
            {t.emptyTitle}
          </p>
          ...
          {penal.length > 0 && (
            ...
            >
              <header className="mb-6 sm:mb-8">
                <p className="text-[10px] sm:text-[11px] tracking-[0.3em] text-noir/40 mb-2">
                  01 · {t.sectionPenal}
                </p>
                <h2
                  id="penal-heading"
                  className="text-[22px] sm:text-[30px] leading-[1.15] font-light border-t border-noir/15 pt-6"
                >
                  {t.sectionPenal}
                </h2>
